...
class Element_By_Element_Search:
    ...
    def measure_patterns(self):
        ...
        for i in range(len(self.pat_array)):
            ...
            while True:
                enum += 1
                start_pat = max(0, int(self.point_range * i + self.N_pts_delete + shift))
                end_pat = min(len(self.POWER_REC), int(self.point_range * (i + 1) - self.N_pts_delete + shift))
