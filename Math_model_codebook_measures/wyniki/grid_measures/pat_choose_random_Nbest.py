...
def przeplywnosc(x, W = 200E6, noise = -69): # x = rec pwr
    if noise > x:
        return 0.0
...
def metric(selections):
    ...
    ret_val =   np.sqrt(np.mean(max_values**2))
    return ret_val
...
while i < ITERATIONS:
    ...
    if m > best:
        best = m
        best_sel = selections.copy()  # Store the best selection
