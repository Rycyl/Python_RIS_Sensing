    ...
    for pattern in patterns:
        ...
        for i, path in enumerate(file_paths, start=1):
            ...
            if not config_data.empty:
                ...
                power_values.append(-100)
        plt.figure(figsize=(10, 6))
        bar_heights = [power - (-100) for power in power_values]
        plt.bar(labels, bar_heights, bottom=-100, color=colors[:len(power_values)])
        plt.xlabel('Path Index and Power')
        plt.ylabel('Power (dBm)')
        plt.title(f'Power Levels for "{pattern}" Across Different Paths at {how_far_from_box} cm')
        plt.ylim([-90, -60]) 
...
output_folder = r'C:\Users\marsieradzka\Desktop\ris\Python_RIS_Sensing\Skrypty_sensing\artykul\wykres\TXdookol_RXdookol\for_one_pattern_four_position_bar'
