...
class Antenna_Geometry_MDEK1001():
    def __init__(self, tag, tx_id, rx_id, a1_id, a2_id):
        #Init TAG device
        self.tag = tag
        #Init ID of devices
        self.tx_id = tx_id
    ...
    def calc_distances(self):
        ...
        self.a = numpy.linalg.norm(self.loc_a1 - self.loc_tag)
        self.b = numpy.linalg.norm(self.loc_tag - self.loc_a2)
        self.c = numpy.linalg.norm(self.loc_tag - self.loc_tx)
        self.d = numpy.linalg.norm(self.loc_tag - self.loc_rx)
        self.e = numpy.linalg.norm(self.loc_a1 - self.loc_tx)
        self.f = numpy.linalg.norm(self.loc_a2 - self.loc_rx)
        return
    ...
    def calc_angles(self, degrees=True):
        ...
        '''
        self.alfa = angle_from_points(self.loc_tag, self.loc_a1, self.loc_tx)
        self.beta = angle_from_points(self.loc_tag, self.loc_a2, self.loc_rx)
        return
    ...
    def get_angles(self, Print_vals=False):
        ...
        """
        line = self.tag.read_line()
        self.loc_a1, self.loc_a2, self.loc_rx, self.loc_tx, self.loc_tag = \
            self.tag.parse_line(line, self.a1_id, self.a2_id, self.rx_id, self.tx_id)
...
if __name__ == "__main__":
    uwb = New_UWB_module()
    devices_ids = ["AAAA", "BBBB", "CCCC", "DDDD"]
